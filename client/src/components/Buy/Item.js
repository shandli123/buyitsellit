					...
					<div className="card box-shadow--8dp">
						<div id={"images" + item._id} className="carousel slide" data-ride="carousel">
							...
							<div className="card-body">
								<h4 className="card-title" style={{color: 'blue'}}>{item.name}</h4>
								...
								<div className="container desc-list">
									...
									<button type="button" className="btn btn-default prod-btn" style={{ backgroundColor: "#38d39f",margin:"15px" }}  onClick={this.handleInterested} data-toggle="tooltip" data-placement="bottom" title={this.state.status === 'Interested' ? 'Remove Interest' : 'Express Interest'} id='mytooltip'>
										{this.state.image}
									</button>

									<div className="card-text int-card-text time"><small className="text-muted">{this.calcTime(this.props.item.timestamp)}</small></div>

									<button type="button" className="btn btn-default" style={{ backgroundColor:"#38d39f" }}  data-toggle="modal" data-target={"#commentsModal" + this.props.item._id}>
